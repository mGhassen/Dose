...
function transformToSnakeCase(data: UpdateLeasingPaymentData): any {
  ...
  if (data.totalAmount !== undefined) {
    result.total_amount = data.totalAmount !== null ? data.totalAmount : null;
  }
...
export async function PUT(
  ...
  try {
    ...
    const parsed = await import('@/shared/zod-schemas').then((m) =>
      m.parseRequestBody(request, m.updateLeasingSchema)
    );
    if (!parsed.success) return parsed.response;
    const body = parsed.data as UpdateLeasingPaymentData;
