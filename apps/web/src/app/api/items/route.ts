  ...
  try {
    ...
    if (includeRecipes && !producedOnly) {
      ...
      const recipesAsItems = (recipesData || []).map((recipe: any) => ({
        ...
        output_quantity: recipe.output_quantity ?? recipe.serving_size,
        serving_size: recipe.output_quantity ?? recipe.serving_size,
    ...
    const items: Item[] = paginatedData.map((row: any) => {
      if (row.instructions != null) {
        return {
          ...
          createdAt: row.created_at,
          updatedAt: row.updated_at,
          outputQuantity: row.output_quantity ?? row.serving_size,
          servingSize: row.output_quantity ?? row.serving_size,
