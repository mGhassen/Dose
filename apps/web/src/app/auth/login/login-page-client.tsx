...
import { useToast, useAuth } from "@kit/hooks";
...
export function LoginPageClient({ loginImages }: LoginPageClientProps) {
  ...
  const handlePrefilledLogin = async (prefillEmail: string, prefillPassword: string) => {
    ...
    setIsLoading(true);
    try {
      await login(prefillEmail, prefillPassword);
      if (!loginError) {
        toast({
          title: "Login successful!",
          description: "Redirecting you to your dashboard...",
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <p className="text-center text-sm text-muted-foreground">
                      Quick Login — Test Different Roles
                    </p>
                    <div className="grid grid-cols-2 gap-2">
                      {(
                        [
                          ["admin@dose.com", "Admin"],
                          ["manager@dose.com", "Manager"],
                          ["user@dose.com", "User"],
                          ["analyst@dose.com", "Analyst"],
                          ["pending@dose.com", "Pending"],
                        ] as const
                      ).map(([addr, label]) => (
                        <Button
                          key={addr}
                          ...
                          size="sm"
                          onClick={() =>
                            handlePrefilledLogin(addr, "password123")
                          }
                          disabled={isLoading}
                          className={`text-xs ${label === "Pending" ? "col-span-2" : ""}`}
                        >
                          {isLoading ? (
                            <>
