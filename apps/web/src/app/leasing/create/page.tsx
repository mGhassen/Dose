...
export default function CreateLeasingPage() {
  ...
  const createLeasing = useCreateLeasingPayment();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createLeasing.mutateAsync({
        name: formData.name,
        type: formData.type,
      ...
      toast.error(error?.message || "Failed to create leasing payment");
    }
  };
  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Create Leasing Payment</h1>
          <p className="text-muted-foreground">Record a new leasing or rental payment</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Leasing Payment Information</CardTitle>
            <CardDescription>Enter the details for this leasing payment</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end space-x-4 pt-6">
                ...
                <Button type="submit" disabled={createLeasing.isPending}>
                  <Save className="mr-2 h-4 w-4" />
                  {createLeasing.isPending ? "Creating..." : "Create Leasing Payment"}
                </Button>
              </div>
            </form>
          </CardContent>
