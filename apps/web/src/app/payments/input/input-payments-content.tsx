...
export default function InputPaymentsContent() {
  const router = useRouter();
  const { data: payments, isLoading } = useActualPayments({ direction: 'input' });
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Badge variant={row.original.isPaid ? "default" : "secondary"}>
          {row.original.isPaid ? "Received" : "Pending"}
        </Badge>
      ),
    },
    {
  ...
  const handleBulkExport = (data: ActualPayment[], type: 'selected' | 'all') => {
    ...
    a.href = url;
    a.download = `input-payments-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Input Payments</h1>
          <p className="text-muted-foreground mt-2">
            Manage all input payments (money coming in) - sales, revenue, etc.
          </p>
        </div>
      </div>
      ...
      <div className="-mx-4">
        <DataTablePage
          title=""
          description=""
          createHref="/payments/input/create"
          data={payments || []}
          columns={columns}
          loading={isLoading}
          ...
          localStoragePrefix="input-payments"
          searchFields={["notes"]}
        />
      </div>
    </div>
