...
interface MetadataEnumDetailPageProps {
  params: Promise<{ id: string }>;
}

export default function MetadataEnumDetailPage({ params }: MetadataEnumDetailPageProps) {
  ...
  const { data: enumItem, isLoading } = useMetadataEnumById(id);
  const { data: enumValues } = useEnumValuesByEnumId(id);
  ...
  const handleCreateValue = async (data: {
    ...
    if (!id) return;
    try {
      await createValueMutation.mutateAsync({ enumId: id, data });
  ...
  const handleUpdateValue = async (valueId: number, data: Partial<MetadataEnumValue>) => {
    if (!id) return;
    try {
      await updateValueMutation.mutateAsync({ enumId: id, valueId, data });
  ...
  const handleDeleteValue = async (valueId: number) => {
    ...
    try {
      await deleteValueMutation.mutateAsync({ enumId: id, valueId });
