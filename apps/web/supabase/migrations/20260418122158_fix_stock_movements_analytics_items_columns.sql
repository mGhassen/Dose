-- Fix stock_movements_analytics to match current schema:
--   items.unit       -> dropped; use sm.unit (text on stock_movements) for per-movement unit label.
--   items.category   -> dropped; replaced by items.category_id -> item_categories(label).
