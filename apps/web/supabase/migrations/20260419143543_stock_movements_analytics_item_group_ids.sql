-- Align stock_movements_analytics item filter with GET /api/stock-movements:
-- canonical group items expand to all member item_ids; the old single p_item_id
-- missed movements stored on non-canonical members.

drop function if exists stock_movements_analytics(timestamptz, timestamptz, bigint, text[]);
...
grant execute on function stock_movements_analytics(timestamptz, timestamptz, bigint[], text[])
