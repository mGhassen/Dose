-- Server-side aggregation for the stock movements analytics page.
-- Returns one JSONB blob with daily activity, breakdowns by type / category /
-- location / reference / item, weekday distribution and a heatmap-friendly
-- daily count series.
...
begin
  ...
  weekday as (
    select
      extract(isodow from movement_date)::int as dow, -- 1=Mon..7=Sun
      count(*) as count,
      sum(quantity) as qty
    from filtered
    group by 1
