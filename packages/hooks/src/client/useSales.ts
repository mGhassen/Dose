...
export function useSales(params?: { page?: number; limit?: number; month?: string; year?: string; type?: string; startDate?: string; endDate?: string }) {
  return useQuery({
    queryKey: ['sales', params],
    queryFn: async () => {
      try {
        const result = await salesApi.getAll(params);
        ...
        if (Array.isArray(result)) {
          const arr = result as import('@kit/types').Sale[];
          return {
            data: arr,
            pagination: {
              page: 1,
        ...
        console.warn('[useSales] Unexpected response format:', result);
        return {
          data: [],
          pagination: {
            page: 1,
        ...
        console.error('[useSales] Error fetching sales:', error);
        throw error;
      }
    },
  });
...
export function useSalesByMonth(month: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await salesApi.getByMonth(month);
...
export function useSalesByType(type: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await salesApi.getByType(type);
      // Extract data from paginated response
      return result?.data || [];
    },
    enabled: !!type,
